...
| 12 | Freee請求書送付ステータスと決済ステータスを同期する | システム | 毎日3:00に同期。結果確認はSAMURAI 経理 |
...
2. 経理またはシステム管理者がSalesforceの `freee請求書を取込` を実行する。スケジュール運用開始後は夜間バッチで自動実行する。
...
- freee送付ステータス・決済ステータスの同期バッチは継続する。
- 経理レポートはSalesforce請求を実績として確認する。
- 営業レポートは契約月次明細を予定データとして確認する。

## 追記: freee取込・入金金額確認手順（2026-06-23更新）
...
1. `Mig_FreeeInvoiceWork__c` のリストビューで、取込エラー、要確認、反映可能の件数を確認する。
2. `要確認` のWorkがある場合は、取引先、契約管理、契約期間、契約月次明細、商品マスタの紐づけを確認する。
3. 修正後、`freee請求書を再検証` を実行する。
4. `反映可能` のWorkを選択し、`freee請求書を一括本反映` を実行する。
5. 請求レコードで、請求金額、税額、入金額、未入金額、送付ステータス、決済ステータス、請求書URLを確認する。
6. 未送付、未入金、取込エラーは経理用レポートまたはダッシュボードで確認する。
...
3. 既存データの入金額・未入金額を補正する必要がある場合のみ、`scripts/apex/backfill-freee-invoice-amounts.apex` を一度だけ実行する。
