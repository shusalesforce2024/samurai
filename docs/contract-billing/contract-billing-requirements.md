...
- 年額按分は、商品別の税抜純額を12分割して算出する。端数は通貨項目の精度に従い、12か月合計と請求純額の差異を検証する。
- 商品・対象月から請求が一意に確定した場合に関連請求を保持し、契約月次明細レポートから請求の決済ステータスを確認できるようにする。
...
| 営業 | 【営業】MRR(取引ベース) | 取引 | 契約開始日から算出した契約開始月（YYYY/MM）別の取引MRR |
...
- 本反映後、商品・対象月から1件に確定でき、別請求が未設定の契約月次明細 `ContractLineItem__c.RelatedInvoice__c` に請求を紐づける。
...
- freee請求書が作成された後、Salesforceはfreee請求書を取り込み、商品・対象月が一意に一致する契約月次明細 `ContractLineItem__c.RelatedInvoice__c` を更新する。期間内の全月次明細へ一律設定しない。
...
- 年払いは、請求明細の税抜金額から値引き・調整明細を含めた商品別純額を算出し、12か月で按分して契約月次明細を作成する。商品マスタの年契約月額按分料金が未設定でも処理を継続する。
...
- 同一契約に既存契約期間と重複する期間を作成しない。期間が重複する場合は `要確認` とする。
...
## 27. 経理確認作業を削減する自動解決要件

### 27.1 自動解決の基本方針

経理担当者が同じ商品判定や不要行の除外を繰り返さないよう、過去の確定結果と機械的に一意な条件を再利用する。一方、取引先・契約・商品が複数候補となる場合は誤請求防止を優先し、推測で確定しない。

### 27.2 商品の自動確定

- 過去730日以内の反映済み請求明細から、正規化したfreee明細名、単価、数量、明細金額が完全一致する確定履歴を検索する。
- 一致する商品が1件だけの場合、その商品を自動確定する。
- 同じ条件に複数商品が存在する場合は自動確定せず `要確認` とする。
- 金額が一致するだけ、または名称が曖昧に類似するだけでは商品を自動確定しない。
- 金額・数量がなく請求額に影響しない備考行は、自動的に請求対象外とする。
- 値引き・割引・調整を示すマイナス明細は、同一請求内の主要商品が1件だけ確定している場合に限り、その商品へ自動的に紐づける。

### 27.3 再取込の整合性

- freee請求を再取得した際、最新のfreee明細に存在せず、Salesforce請求明細へ未反映の古い明細Workは削除する。
- Salesforce請求明細へ反映済みの明細Workは削除しない。
- 反映済みWorkを再取得しても、ステータスを `取得済み` に戻さず、作成済み請求との関連を維持する。

### 27.4 契約月次明細と請求の関連付け

- 請求明細の商品、請求対象日、契約期間から契約月次明細を検索し、候補が1件の場合だけ `RelatedInvoice__c` を設定する。
- 候補が複数ある場合は先頭レコードを採用せず、関連請求を自動設定しない。
- `RelatedInvoice__c` に別の請求が設定済みの場合は上書きしない。
- 年一括請求を契約期間内の12か月すべてへ一律設定しない。請求実績の全件確認は `Invoice__c.ContractPeriod__c` の関連リストまたは請求ベースのレポートで行う。

### 27.4.1 請求作成後の契約分類補完

- 契約に紐づく請求明細が作成または更新されたとき、契約の商品カテゴリ、プランカテゴリ、アカウント数の空欄を自動補完する。
- 商品カテゴリとプランカテゴリは、請求明細名および商品マスタ名から判定する。
- アカウント数は明細内の「Nアカウント」の記載を優先し、それがない場合は請求明細の数量を使用する。
- 単位が月・年を示す場合、または年一括商品の数量が月数を表す場合は、その数量をアカウント数として使用しない。
- 契約に設定済みの値は請求明細から上書きしない。
- 取消請求および初期費用明細は判定対象外とする。

### 27.5 経理担当者が確認する対象

次のいずれかに該当するWorkだけを経理担当者の確認対象とする。

| 確認理由 | 経理担当者の確認内容 |
|---|---|
| 商品候補なし・複数 | 正しい商品マスタを選択する |
| 取引先候補なし・複数 | freee取引先IDとSalesforce取引先を確認する |
| 有効契約候補が複数 | 請求を紐づける契約管理を選択する |
| 契約期間が重複 | 正しい契約期間を確認し、不要な期間を修正する |
| 請求対象日なし | freee請求書の対象期間または請求日を確認する |
| 金額不一致 | 税区分、値引き、調整明細および請求合計を確認する |

自動確定されたWorkは経理担当者の個別確認を不要とし、`反映可能` の一覧で件数と合計金額を確認したうえで一括反映できること。
