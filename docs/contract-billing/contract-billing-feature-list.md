...
| 資産 | 機能 |
|---|---|
| `FreeeInvoiceContractProvisioningService` | 取引先・確定商品・対象日で既存契約を一意判定し、取引先別例外ルールを商品共通ルールより優先して、必要な契約・契約期間・契約月次明細を冪等作成。対象期間未取得時は請求月を使用し、既存契約が複数一致する場合は新規作成せず要確認 |
