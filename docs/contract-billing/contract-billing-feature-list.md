...
- 初期費用は初月のみ作成する。
- 年契約の月額は、請求取込時は請求明細の商品別税抜純額を12分割して算出する。営業受注からの初回作成は既存の商品マスタ料金を利用する。
- 契約月次明細レポートで決済状況を見られるよう、商品・対象月から一意に確定できる場合に関連請求を設定する。
...
年契約の場合も、商品・対象月から一意に特定できる契約月次明細だけが年一括請求を参照する。請求の全履歴は契約期間の請求関連リストで確認する。
...
請求・請求明細・Freee請求書はSalesforce側の更新請求バッチでは作成せず、freee側の自動作成・自動送付を正とする。
freee請求書取込後、商品・対象月から一意に特定できる契約月次明細だけへ関連請求を設定する。
同一契約・同一期間の契約期間が存在する場合は作成しない。
```
...
```text
決済ステータス != 決済済み
Freee連携済み、またはFreee連携ステータス = Failed
取消ステータス != 取消済
通常対象の支払期日 = 前月1日〜当月末
```
...
4. 最終同期日時を更新する
5. 20件処理後、残件があれば2分後に次の20件を自動実行する
6. 429の場合は後続呼出しを止め、freeeのリセット時刻以降に自動再開する
7. 失敗時は同期エラー内容を保存する
8. 1夜最大1,000件とし、超過分は翌日へ繰り越して警告ログを保存する
```
...
FreeeInvoiceStatusSyncThrottledQueueable
FreeeInvoiceStatusSyncThrottledScheduler
```
...
Freee送付・決済ステータス同期バッチ実行時刻: 毎日 0:10開始
旧更新請求バッチ: 通常運用では停止
税率: 10%
税端数処理: 切り上げ
Freee APIエンドポイント
...
FreeeInvoiceStatusThrottleTest
...
| 【営業】MRR(取引ベース) | 取引 | 契約開始月（YYYY/MM）別に取引MRRを集計 |
...
| `FreeeInvoiceStatusSyncBatch` | 継続 | 毎日0:10に動的同期を開始し、通常対象と過去のFailed請求を20件単位で同期するため |
| `Mig_FreeeInvoiceWorkRetentionBatch` | 継続 | 毎月1日3:00に、反映済みかつ請求作成済みで2か月超のfreee請求移行Workを削除するため |
...
| `FreeeInvoiceStatusSyncBatch` | 有効 | 毎日0:10に動的同期を開始し、支払期日が前月1日〜当月末の請求と過去のFailed請求について、freee側の送付ステータス・決済ステータス・金額情報をSalesforceへ反映するため |
| `ContractRenewalInvoiceBatch` | 停止 | Salesforce起点の更新請求作成はfreee自動作成と二重請求になるため |
| `Mig_FreeeInvoiceWorkRetentionBatch` | 有効 | 毎月1日3:00に、請求作成済みの反映済みWorkを2か月保持後に削除するため |
...
| 資産 | 内容 |
|---|---|
| `FreeeInvoiceContractRule__c` | 通常は商品・件名キーワード・請求区分を共通管理し、取引先固有の例外時だけ取引先、Freee取引先ID、既存契約を管理 |
| `Contract__c.ContractNature__c` | 継続契約・単発契約を識別 |
| `Contract__c.BillingControlSource__c` | Salesforce・Freeeのどちらが更新請求を管理するか識別 |
| `Contract__c.CreationSource__c` | 営業受注・Freee請求取込を識別 |
| `Contract__c.AutoCreatedFromFreee__c` | freee請求取込による自動作成契約を識別 |
| `Mig_FreeeInvoiceWork__c.ResolutionAction__c` | 既存紐付け、期間補完、契約自動作成、要確認を保持 |
| `Mig_FreeeInvoiceWork__c.ProvisioningMessage__c` | 自動処理結果・停止理由を日本語で保持 |

### Apex

| 資産 | 機能 |
|---|---|
| `FreeeInvoiceContractProvisioningService` | 取引先・確定商品・対象日で既存契約を一意判定し、取引先別例外ルールを商品共通ルールより優先して、必要な契約・契約期間・契約月次明細を冪等作成。対象期間未取得時は請求月を使用し、既存契約が複数一致または既存期間と重複する場合は新規作成せず要確認。年払いの月額は請求明細の商品別純額を12分割して算出 |
| `Mig_FreeeInvoiceValidator` | 商品解決後に契約自動補完を実行し、再度参照解決して本反映可否を判定 |
| `ContractRenewalInvoiceBatch` | 請求管理元がFreeeの契約を更新請求作成対象から除外 |
| `Mig_FreeeInvoiceWorkRetentionBatch` | 請求作成済みで2か月超の反映済みWorkだけを、明細Workとともに削除する |
| `Mig_FreeeInvoiceWorkRetentionBatchTest` | 保持期間、対象外ステータス、作成済み請求未設定、明細Work連動削除、月次スケジュールを検証 |
| `FreeeInvoiceContractProvisionTest` | 継続契約、単発契約、既存契約の期間補完、複数ルール停止、共通ルールの取引先分離、取引先別例外ルール優先、商品一致による既存契約選択、既存契約複数一致時の停止、対象期間未取得時の請求月フォールバック、重複防止を検証 |

### 画面・権限

| 対象 | 内容 |
|---|---|
| SAMURAIアプリ | Freee請求契約ルールタブを追加 |
| リストビュー | `商品共通ルール` と `取引先別例外ルール` を分離して表示 |
| 入力規則 | 共通ルールの商品未設定、既存契約・Freee取引先IDの設定、自動作成未許可での有効化を防止 |
| 経理 | ルールの作成・編集・参照、Work解決結果の確認が可能 |
| 営業 | 契約区分、請求管理元、作成元、Freee自動作成有無を参照可能 |
| システム管理者 | ルール・契約・Workの全項目を管理可能 |

## freee請求Workの自動解決・再取込整合性機能

### Apex

| 資産 | 機能 |
|---|---|
| `Mig_FreeeInvoiceProductResolver` | 過去730日の確定履歴から、正規化明細名・単価・数量・明細金額が完全一致し、商品が一意な場合に自動確定。金額ゼロの備考行を自動除外し、主要商品が一意な場合だけ値引き・調整明細を同商品へ紐づける |
| `Mig_FreeeInvoiceWorkService` | freee再取込時に最新レスポンスに存在しない未反映の古い明細Workを削除。反映済みWorkのステータス・作成済み請求を維持し、税込入力の税額と税抜金額を整合させる |
| `FreeeInvoiceContractProvisioningService` | 年払いの契約月次明細金額を請求明細の税抜純額から12分割して算出。商品マスタの按分料金を必須とせず、既存契約期間との重複を検出して停止する |
| `Mig_FreeeInvoiceFinalizeService` | 商品・対象月で契約月次明細が一意に解決した場合だけ関連請求を設定。既存の別請求を上書きせず、年一括請求を期間内の全月次明細へ一律設定しない |
| `ContractClassBackfillService` | 請求明細作成・更新後、対象の有効契約だけを検索し、商品マスタ名・明細名・数量から商品カテゴリ、プランカテゴリ、アカウント数の空欄を補完。設定済み値、取消請求、初期費用を除外し、年払の月数をアカウント数として扱わない |
| `InvoiceLineTrigger` | 通常請求・freee請求取込の双方で、請求明細保存後に契約分類補完を起動 |
...
|---|---|
| 確定履歴が一意に一致 | 商品を自動確定 |
| 金額ゼロの備考行 | 請求対象外として自動除外 |
| 主要商品が1件の値引き・調整行 | 主要商品へ自動紐づけ |
| 商品、取引先、契約、月次明細の候補が複数 | `要確認` とし推測で確定しない |
| 既存契約期間と重複 | `要確認` とし新規期間を作成しない |
| `反映可能` | 経理担当者が件数・合計金額を確認して一括反映 |

### テスト

| テストクラス | 主な検証 |
|---|---|
| `Mig_FreeeInvoiceMigrationTest` | 確定履歴学習、備考行除外、値引き行紐づけ、再取込時の古い明細削除、税込税額、曖昧な月次明細への誤紐づけ防止 |
| `FreeeInvoiceContractProvisionTest` | 商品マスタ按分料金なしの年払いMRR算出、既存契約期間との重複防止 |
